...
def process_export_job(job_id: str, sla_artifact_id: str):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 4. Export via Scribus (real if configured)
            pdf_path = output_dir / f"output_{job_id}.pdf"
            png_paths = []

            export_mode = os.environ.get("SCRIBUS_EXPORT_MODE", "dummy").lower()
            scribus_exe = os.environ.get("SCRIBUS_EXE")
            export_script = os.environ.get(
                "SCRIBUS_EXPORT_SCRIPT",
                str(Path(__file__).resolve().parent / "scribus_export.py"),
            )
            allow_fallback = os.environ.get("SCRIBUS_EXPORT_FALLBACK", "1") == "1"

            if export_mode == "real":
                if not scribus_exe or not Path(scribus_exe).exists():
                    raise RuntimeError("SCRIBUS_EXE not set or missing for real export")
                if not Path(export_script).exists():
                    raise RuntimeError(f"Scribus export script not found: {export_script}")
                ...
                _log_job(db, job_uuid, "INFO", f"Running Scribus export: {' '.join(cmd)}")
                proc = subprocess.run(cmd, capture_output=True, text=True)
                if proc.returncode != 0:
                    _log_job(db, job_uuid, "ERROR", "Scribus export failed", {"stderr": proc.stderr})
                    if not allow_fallback:
                        raise RuntimeError(f"Scribus export failed: {proc.stderr}")
                else:
                    try:
                    ...
                    if not result or not result.get("success"):
                        if not allow_fallback:
                            raise RuntimeError(f"Scribus export did not return success: {proc.stdout}")
                    else:
                        pdf_path = Path(result["pdf_path"])
                        png_paths = [Path(p) for p in (result.get("png_paths") or [])]

            if export_mode != "real" or (not pdf_path.exists()):
                # Dummy-Export (fallback)
                _log_job(db, job_uuid, "WARN", "Using dummy export - Scribus export not implemented or failed")
                try:
            ...
            for png_path in png_paths:
                ...
                png_artifacts.append({"page": page_num, "artifact_id": str(png_artifact_id), "uri": png_storage_uri})
